...
// 8-hour shift | Session1: 09:30-13:30 | Break 45min | Session2: 14:15-17:30
let settings = {
  workHours: 8,
  startTime: '09:30',
  endTime: '17:30',
...
function calcTotalSeconds(sessions) {
  return sessions.reduce((t,s)=>{
    if(s.in&&s.out){const[h1,m1]=s.in.split(':').map(Number),[h2,m2]=s.out.split(':').map(Number);return t+(h2*3600+m2*60)-(h1*3600+m1*60);}
    return t;
  },0);
}
function secsToStr(s){return `${Math.floor(s/3600)}h ${String(Math.floor((s%3600)/60)).padStart(2,'0')}m`;}
...
// LEAVE: employee can apply; admin views only (no approval)
...
app.post('/api/checkin',async(req,res)=>{
  ...
  if(!rec) rec=new Attendance({empId,name,date,sessions:[],totalWork:'0h 00m',status:'present'});

  if(rec.sessions.find(s=>s.in&&!s.out))return res.json({error:'Already clocked in'});
  if(rec.sessions.length>=2)return res.json({error:'Both sessions already completed for today'});

  const sessionIndex=rec.sessions.length; // 0=S1, 1=S2

  // Enforce 45-min break between sessions
  if(sessionIndex===1){
  ...
  if(sessionIndex===0){
    ...
    if(nowMins>graceEnd){isLate=true;lateNote=`Late S1 check-in at ${checkIn}`;rec.status='late';}
  } else {
    const s2Grace=toMins(settings.session2Start)+settings.graceMins;
    if(nowMins>s2Grace){isLate=true;lateNote=`Late S2 check-in at ${checkIn}`;if(rec.status!=='late')rec.status='late';}
  }
